        ...
        <div className="flex space-x-8">
          {hariyaliTeej.slice(0, 6).map((data, index) => (
            <Link to={`/product/${data.id}`} key={index}>
              <div
                key={data.id}
                className="flex-shrink-0 flex flex-col gap-5 items-center justify-center border-2 border-red-400 p-3 h-full rounded-md snap-center transition-all 3s ease-in-out"
